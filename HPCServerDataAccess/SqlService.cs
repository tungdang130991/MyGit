...
namespace HPCServerDataAccess
{
	...
	public class SqlService
	{
		...
		public SqlService()
        {
            //ConnectionString = ConfigurationManager.AppSettings[0];
            HPCShareDLL.Configuration config = HPCShareDLL.Configuration.GetConfig();
            Provider sqlProvider = (Provider)config.Providers[config.DefaultProvider];
            _connectionString = sqlProvider.Attributes["connectionString"];
            _commandTimeout = Convert.ToInt16(sqlProvider.Attributes["commandTimeout"]);
		}
		...
		public SqlDataReader ExecuteSqlReaderParam(string sql)
		{
			SqlDataReader reader;
			SqlCommand cmd = new SqlCommand();
			this.Connect();
		...
		public DataSet ExecuteSqlDataSetParam(string sql)
		{
			SqlCommand cmd = new SqlCommand();
			this.Connect();
			SqlDataAdapter da = new SqlDataAdapter();
		...
		{
            parameter.Value= PrepareSqlValue(parameter.Value, true);
