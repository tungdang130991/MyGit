...
(function($) {
    ...
    initialize: function(options) {
      ...
      this.options = $.extend(this.getOptions(), options);
			...
			$('#BoxOverlay').css({
				...
				height          : '100%',    //$(document).height(),    //Nozel : made 100% coz was causing problem when browserwindow resized
				width           : '100%'    //$(document).width()       //Nozel : made 100% coz was causing problem when browserwindow resized
			}).hide();
      ...
      this.Box.draggable({ opacity: 0.35, cursor: 'move' });   // Nozel: Added To make the Alert-box Dragable
 ...
 ///////        $('embed, object, select').css({ 'visibility' : 'hidden' });      Nozel: Not sure the need of this (removed as was delibrately hiding the controls)
        }
        ...
        if(this.i==1) {
          ...
          $('#BoxOverlay').fadeOut(this.options.closeDuration, $.bind(this, function(){
            $('#BoxOverlay').hide();
            if (!$.support.maxHeight) { //IE6
//////////      $('embed, object, select').css({ 'visibility' : 'visible' });  Nozel: Compensate for above commented line
            }
