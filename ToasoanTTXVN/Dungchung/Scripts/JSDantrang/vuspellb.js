...
"<b>Soát l&#7895;i chính t&#7843; - Tìm và thay th&#7871;</b></td></tr>"+
"<tr><td align=right><b>Tìm th&#7845;y:&nbsp;</b></td>" +
...
"<tr><td align=right><b>G&#7907;i ý khác:&nbsp;</b></td>" +
...
"value='Tìm ti&#7871;p' onclick='iCSpell(this.form,2);'></td></tr>"+
"</table></form></div>"+
"<script defer>"+
...
function stepCS(form, cmd) {
  ...
  if (cmd == 0) {
    ...
    if (this.found) {
      ...
      this.counter2++;
      theTyper.txtarea.value = this.value.substring(1,this.value.length-1);
    }
...
function checkThis(form) {
  ...
  if (typeof(rec[1])=='object') {
    ...
    for(var i= 0; i < rec[1].length; i++) {
      var option = form.document.createElement("OPTION");
      form.slist.options.add(option); 
      option.text= rec[1][i];
      option.Value = i;
    }
