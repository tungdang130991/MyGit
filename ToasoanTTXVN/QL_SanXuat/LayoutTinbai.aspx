<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/ITCv2.Master" AutoEventWireup="true"
    CodeBehind="LayoutTinbai.aspx.cs" Inherits="ToasoanTTXVN.QL_SanXuat.LayoutTinbai"
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
                ...
                if (trang_bao != "") {
                    ...
                    if (arr.length > 0) {
                        for (var j = 0; j < arr.length; j++) {
                            for (var k in arr[j]) {
                                URL_Get = "LayoutHandler.ashx?masobao=" + maso_bao + "&trangbao=" + trang_bao + "&title=" + arr[j][0] + "&width=" + arr[j][1] + "&height=" + arr[j][2] + "&left=" + arr[j][3] + "&top=" + arr[j][4] + "&matinbai=" + arr[j][5] + "&macv=" + arr[j][6] + "&maqc=" + arr[j][7] + "&type=2";
                            }
    ...
    <script type="text/javascript" language="javascript">
            ...
            for (i = 0; i < obj.length; i++) {
                if (obj[i].className == 'drsElement') {
                    if (document.getElementById('txtTB' + obj[i].id) != null && document.getElementById('txtTB' + obj[i].id).value != "")
                        ...
                        MaTinbai = 0
                    arr[counraArr] = new Array(obj[i].title, obj[i].style.width, obj[i].style.height, obj[i].style.left, obj[i].style.top, MaTinbai, 0, 0);
                    counraArr++;
                }
            }
            return arr;
        ...
        function SelectItem(trangbao) {
            ...
            var sLink = 'BindLayoutTB.aspx?mabao=' + sobao + '&trangbao=' + trangbao
            var $loader = $('#ctl00_MainContent_Canvas');
            $loader.load(sLink);
            var sLink3 = 'LoadTinbai.aspx?maAnpham=<%=Session["LayMaAnPham"]%>&mabao=' + sobao + '&trangbao=' + trangbao;
            var $loader3 = $('#DIVChonbaivietHolderID');
            $loader3.load(sLink3);
